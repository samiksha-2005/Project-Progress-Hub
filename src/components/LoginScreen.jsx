...
function LoginScreen({ onLogin, showToast }) {
  ...
  return (
    <div className="min-h-screen px-4 py-4 sm:px-6 lg:px-0 lg:py-6">
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-6xl overflow-hidden rounded-[28px] border border-line/80 bg-surface/85 shadow-[0_30px_90px_-35px_rgba(0,0,0,0.9)] backdrop-blur-xl lg:min-h-[calc(100vh-3rem)]">
      <div className="flex w-full flex-col lg:flex-row">
      <section className="hidden lg:flex relative overflow-hidden border-r border-line/70 flex-col justify-between p-10 bg-surface/90 login-art">
      ...
      <section className="flex items-center justify-center p-6 sm:p-8 bg-[linear-gradient(180deg,rgba(255,255,255,0.02),transparent)] login-panel">
        <div className={`w-full max-w-[340px] p-7 rounded-[3px] bg-surface/95 border border-line shadow-[0_18px_45px_-20px_rgba(0,0,0,0.65)] relative login-card ${shake ? 'animate-shake' : ''}`}>
          {/* Corner brackets */}
          ...
          <form onSubmit={handleSubmit}>
            <div className={`mb-3.5 field ${emailError ? 'has-error' : ''}`}>
              <label className="block font-mono text-[10.5px] uppercase tracking-wider text-muted mb-1.5">Email</label>
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className={`w-full bg-surface-2 border ${emailError ? 'border-coral' : 'border-line'} text-paper px-2.5 py-2 rounded-[3px] text-[13px] focus:border-amber-dim focus:outline-none transition-colors`}
                placeholder="you@company.com"
              />
              {emailError && <div className="field-error">Enter a valid email address.</div>}
            </div>

            <div className={`mb-3.5 relative field pw-field ${passwordError ? 'has-error' : ''}`}>
              <label className="block font-mono text-[10.5px] uppercase tracking-wider text-muted mb-1.5">Password</label>
              <input
                type={showPassword ? 'text' : 'password'}
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                className={`w-full bg-surface-2 border ${passwordError ? 'border-coral' : 'border-line'} text-paper px-2.5 py-2 rounded-[3px] text-[13px] focus:border-amber-dim focus:outline-none transition-colors`}
                placeholder="••••••••"
              />
              <button
                type="button"
              ...
              {passwordError && <div className="field-error">Password must be at least 4 characters.</div>}
            </div>

            <div className="flex items-center justify-between mb-4 text-xs login-row-between">
